"""
Creates a brick grid of individual closed rectangular cells within a region.

Inputs:
    boundary: Geometry (item access, crv)
    size_x: Number (item access, float)
    size_y: Number (item access, float)
    cell_shift: Number (item access, float)
...
def create_closed_brick_cells(region_geometry, size_x_val, size_y_val, cell_shift_val):
    ...
    for row in range(-2, cells_y + 4):
        ...
        for col in range(-2, cells_x + 4):
            cx = p_min.X + (actual_dx / 2.0) + (col * actual_dx) + shift

            # Check if the shifted cell still fits strictly within the bounding box
...
if 'boundary' in globals() and 'size_x' in globals() and 'size_y' in globals() and 'cell_shift' in globals():
    rectgrid, bnd_rect = create_closed_brick_cells(boundary, size_x, size_y, cell_shift)