...
A user will input a collection of 3D "folded grid" geometries which must be unrolled/flattened FAITHFULLY to represent the actual dimensions of the folds and their angles, but in 2-dimensions so the linework can be used to cut/fold the design in production/laser cutting. please reference the existing folding scripts/code to see how the geometry behaves in the model/grasshopper canvas.

## Inputs:

grid cells: base grid cells with 3d folds
